        ...
        <div class="">
            <div class="bg-slate-200">
                <div class="p-6 text-gray-900 space-y-10">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                @foreach ($enrollments as $enrollment)
                                    @php
                                        ...
                                        $eStatus  = $enrollmentStatusLabels[$enrollment->status] ?? ['label' => $enrollment->status, 'class' => 'bg-gray-100 text-gray-600'];
                                        $sStatus  = $sessionStatusLabels[$session?->status] ?? ['label' => $session?->status, 'class' => 'bg-gray-100 text-gray-600'];
                                    @endphp
                                    <div class="flex flex-col bg-gray-50 px-6 py-4 rounded-sm">
                                        <div class="flex-1">
                                            <div class="flex items-start justify-between gap-2 mb-2">
                                                ...
                                                <span class="shrink-0 text-xs font-medium px-2 py-0.5 rounded-full {{ $eStatus['class'] }}">
                                                    {{ $eStatus['label'] }}
                                                </span>
                                            </div>
                                            ...
                                            @if ($enrollment->progress > 0)
                                                <div class="mb-3">
                                                    ...
                                                    <div class="w-full bg-gray-200 rounded-full h-1.5">
                                                        <div class="bg-gray-700 h-1.5 rounded-full" style="width: {{ $enrollment->progress }}%"></div>
                                                    </div>
                                                </div>
                                            @endif
                                        </div>

                                        <div class="mt-auto pt-3 border-t border-gray-200 space-y-1 text-sm text-gray-500">
                                            @if ($session)
                                                <div>
                                                    ...
                                                    <span class="ml-1 text-xs font-medium px-1.5 py-0.5 rounded-full {{ $sStatus['class'] }}">{{ $sStatus['label'] }}</span>
                                                </div>
                                                @if ($session->location)
                    ...
                    {{-- ── Внешние курсы (одобренные заявки) ────────────────────── --}}
                    <div>
                                    ...
                                    <div class="flex flex-col bg-gray-50 px-6 py-4 rounded-sm">
                                        <div class="flex-1">
                                            <div class="flex items-start justify-between gap-2 mb-2">
                                                ...
                                                <span class="shrink-0 text-xs font-medium px-2 py-0.5 rounded-full bg-green-100 text-green-800">
                                                    Одобрена
                                                </span>
                                            </div>
                                        ...
                                        <div class="mt-auto pt-3 border-t border-gray-200 space-y-1 text-sm text-gray-500">
                                            ...
                                            @if ($exRequest->program_link)
                                                <div>
                                                    <a href="{{ $exRequest->program_link }}" target="_blank" rel="noopener noreferrer"
