<x-app-layout>
                    ...
                    <div class="bg-gray-50 px-6 py-4 rounded-sm space-y-3">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            ...
                            <div>
                                ...
                                <p>{{ \Carbon\Carbon::parse($session->start_date)->format('d.m.Y') }} – {{ \Carbon\Carbon::parse($session->end_date)->format('d.m.Y') }}</p>
                            </div>
                            @if ($session->location)
                                <div>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Сертификат</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach ($enrollments as $enrollment)
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-4 py-3">
                                                    <form method="POST" action="{{ route('trainer-enrollments.update-status', $enrollment) }}"
                                                          class="inline-block">
                                                        @csrf
                                                        @method('PATCH')
                                                        <select name="status" onchange="this.form.submit()"
                                                            class="text-xs border-gray-300 rounded px-2 py-1 focus:border-indigo-500 focus:ring-indigo-500">
                                                            <option value="registered" @selected($enrollment->status === 'registered')>Записан</option>
                                                            <option value="in_progress" @selected($enrollment->status === 'in_progress')>В процессе</option>
                                                            <option value="completed" @selected($enrollment->status === 'completed')>Завершён</option>
                                                            <option value="failed" @selected($enrollment->status === 'failed')>Не сдан</option>
                                                            <option value="cancelled" @selected($enrollment->status === 'cancelled')>Отменён</option>
                                                        </select>
                                                    </form>
                                                </td>
                                                <td class="px-4 py-3">
                                                ...
                                                <td class="px-4 py-3">
                                                    @if ($enrollment->status === 'completed' || $enrollment->status === 'failed')
                                                        @if ($enrollment->certificate_path)
                                                            <a href="{{ Storage::url($enrollment->certificate_path) }}" target="_blank"
                                                               class="text-xs text-indigo-600 hover:text-indigo-800 underline">
                                                                Просмотр
                                                            </a>
                                                        @else
                                                            <button type="button" onclick="openCertificateModal({{ $enrollment->id }})"
                                                                class="text-xs text-indigo-600 hover:text-indigo-800 underline">
                                                                Загрузить
                                                            </button>
                                                        @endif
    ...
    <div id="certificateModal" style="display: none;" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
